...
begin
    select count(*)
    INTO IS_TRUE
    from all_users where username='APP_ADMIN';
    IF IS_TRUE > 0
    THEN
...
END;
/
create user APP_ADMIN identified by November2022 DEFAULT TABLESPACE data_ts QUOTA UNLIMITED ON data_ts ; 
GRANT CONNECT, RESOURCE TO APP_ADMIN with admin option; 
--GRANT CONNECT TO APP_ADMIN;  
grant create view, create procedure, create sequence, CREATE USER, DROP USER to APP_ADMIN with admin option;  
--**********************************************************************************



--Execute from APP_ADMIN
set SERVEROUTPUT on;
...
Exec manage_objects.drop_objects('CUSTOMER_WISE_OFFER','view');
Exec manage_objects.drop_objects('PRODUCT_WISE_MONTHLY_SALES','view');
Exec manage_objects.drop_objects('TOTAL_MONTHLY_SALES','view');
Exec manage_objects.drop_objects('MONTHLY_CLAIMS_PROCESSED','view');
Exec manage_objects.drop_objects('CANCELLED_DELIVERIES','view');
Exec manage_objects.drop_objects('CUSTOMER_WISE_MONTHLY_INSURANCE_SALES','view');
Exec manage_objects.drop_objects('PRODUCT_WISE_MONTHLY_INSURANCE_SALES','view');


--exec MANAGE_USERS.drop_users('APP_ADMIN');
...
/
---CUSTOMER WISE OFFER REPORT
create or replace view CUSTOMER_WISE_OFFER as
select cust.CUSTOMER_ID, cust_cat.customer_category_DESC , cust.CUSTOMER_FNAME || ' ' || cust.CUSTOMER_LNAME as customer_name,
Offers.OFFER_DESC
from customer cust
inner join customer_category cust_cat
on cust.customer_category_ID = cust_cat.customer_category_ID
inner join Offers
on Offers.customer_category_ID = cust_cat.customer_category_ID;
/

--PRODUCT WISE MONTHLY SALES REPORT
create or replace view PRODUCT_WISE_MONTHLY_SALES as
select prod.product_ID, prod.Product_NAME, pcat.Product_CATEGORY_DESC,
to_char(ORDER_DATE,'Month') as "MONTH" ,
extract(year from ORDER_DATE)as "YEAR",
sum(ORDER_PRICE)as "TOTAL SALES"
from product prod
inner join product_category pcat on
prod.Product_CATEGORY_ID = pcat.Product_CATEGORY_ID
inner join orders ord on
ord.Product_ID = prod.Product_ID
inner join courier crr on
ord.orders_id = crr.orders_id
group by prod.product_ID, prod.Product_NAME, pcat.Product_CATEGORY_DESC,
...
---MONTHLY CLAIMS PROCESSED REPORT
create or replace view MONTHLY_CLAIMS_PROCESSED as
select extract(year from Resolve_DATE) year, to_char(Resolve_DATE,'Month')as month,
prd.PRODUCT_NAME,ccat.Claim_Description, count(*)as Claims_Processed_Count
from CLAIM claim
inner join CLAIM_CATEGORY ccat
on claim.CLAIM_CATEGORY_ID = ccat.CLAIM_CATEGORY_ID
inner join orders ord on
ord.orders_id = claim.orders_id
inner join product prd on
ord.PRODUCT_ID = prd.PRODUCT_ID
where
Resolve_date <= sysdate
group by
extract(year from Resolve_DATE) , to_char(Resolve_DATE,'Month'),
prd.PRODUCT_NAME,ccat.Claim_Description
order by extract(year from Resolve_DATE) desc;
/
---CANCELLED DELIVERIES REPORT
create or replace view CANCELLED_DELIVERIES as
select cust.customer_ID,cust.customer_fname ||' ' ||cust.customer_lname as customer_name ,prd.Product_Name "product_ordered",
ord.ORDERS_ID,
case crr.DELIVERY_STATUS
when 'Y' then 'DELIVERED'
when 'N' then 'CANCELLED'
end as DELIVERY_STATUS
from courier crr
inner join orders ord
on crr.orders_id = ord.orders_id
inner join product prd
on ord.product_id = prd.product_ID
inner join customer cust
on cust.customer_ID = ord.Customer_ID;
/
---CUSTOMER WISE MONTHLY INSURANCE SALES REPORT
create or replace view CUSTOMER_WISE_MONTHLY_INSURANCE_SALES as
select extract(year from ins.INSURANCE_START_DATE) as year_sale, to_char(ins.INSURANCE_START_DATE,'Month')as month_sale,
cust.customer_ID,cust.customer_fname ||' ' ||cust.customer_lname as CUSTOMER_NAME ,
prd.Product_Name,
ins.Insurance_ID, sum(insp.Insurance_PRICE) Total_Insurance_Price
from insurance ins
inner join orders ord
on ins.Orders_ID = ord.Orders_ID
inner join customer cust
on cust.customer_ID = ord.Customer_ID
inner join product prd
on ord.product_id = prd.product_ID
inner join Insurance_price insp
on ins.INSURANCE_PRICE_ID = insp.INSURANCE_PRICE_ID
group by extract(year from ins.INSURANCE_START_DATE) ,
to_char(ins.INSURANCE_START_DATE,'Month'),
cust.customer_ID,cust.customer_fname ||' ' ||cust.customer_lname  ,
prd.Product_Name,
ins.Insurance_ID
order by extract(year from ins.INSURANCE_START_DATE) desc;
/
--PRODUCT WISE MONTHLY INSURANCE SALES REPORT
create or replace view PRODUCT_WISE_MONTHLY_INSURANCE_SALES as
select extract(year from ins.INSURANCE_START_DATE) year, to_char(ins.INSURANCE_START_DATE,'Month')as month,
prd.Product_Name,
sum(Insurance_PRICE) Total_Insurance_Price
from insurance ins
inner join orders ord
on ins.Orders_ID = ord.Orders_ID
inner join product prd
...
on ins.Insurance_PRICE_ID = insp.Insurance_PRICE_ID
group by extract(year from ins.INSURANCE_START_DATE) ,
to_char(ins.INSURANCE_START_DATE,'Month'), to_char(ins.INSURANCE_START_DATE,'mm'),
prd.Product_Name
order by extract(year from ins.INSURANCE_START_DATE)  desc,
to_char(ins.INSURANCE_START_DATE,'mm') desc;
...
--create user APP_ADMIN identified by November2022 DEFAULT TABLESPACE data_ts QUOTA UNLIMITED ON data_ts ;  
--GRANT CONNECT TO APP_ADMIN;  
--grant create view, create procedure, create sequence, CREATE USER to APP_ADMIN with admin option;  
--GRANT CONNECT, RESOURCE TO APP_ADMIN with admin option;  *********************
--grant create session grant any privilege to APP_ADMIN;   
...
GRANT execute on APP_ADMIN.CUST_MANAGE_ORDER_CLAIM to ALEX;   
--CUSTOMER YASH  
...
GRANT execute on APP_ADMIN.CUST_MANAGE_ORDER_CLAIM to YASH;   
--CUSTOMER JACK  
...
GRANT execute on APP_ADMIN.CUST_MANAGE_ORDER_CLAIM to JACK;   
--CUSTOMER MEENA  
...
GRANT execute on APP_ADMIN.CUST_MANAGE_ORDER_CLAIM to MEENA;   
...
GRANT execute on APP_ADMIN.CLAIM_MANAGER to CLAIM_MANAGER_ADMIN;   
...
GRANT execute on APP_ADMIN.COURIER_MANAGER to COURIER_MANAGER_ADMIN;   
...
GRANT execute on APP_ADMIN.MANAGE_INVENTORY to INVENTORY_MANAGER_ADMIN;   

GRANT select on APP_ADMIN.CUSTOMER_WISE_OFFER to SALES_MANAGER_ADMIN;
GRANT select on APP_ADMIN.PRODUCT_WISE_MONTHLY_SALES to SALES_MANAGER_ADMIN;
GRANT select on APP_ADMIN.TOTAL_MONTHLY_SALES to SALES_MANAGER_ADMIN;
GRANT select on APP_ADMIN.CANCELLED_DELIVERIES to SALES_MANAGER_ADMIN;
GRANT select on APP_ADMIN.CUSTOMER_WISE_MONTHLY_INSURANCE_SALES to SALES_MANAGER_ADMIN;
GRANT select on APP_ADMIN.PRODUCT_WISE_MONTHLY_INSURANCE_SALES to SALES_MANAGER_ADMIN;
GRANT select on APP_ADMIN.MONTHLY_CLAIMS_PROCESSED to CLAIM_MANAGER_ADMIN;
GRANT select on APP_ADMIN.CUSTOMER_WISE_MONTHLY_INSURANCE_SALES to CLAIM_MANAGER_ADMIN;
GRANT select on APP_ADMIN.PRODUCT_WISE_MONTHLY_INSURANCE_SALES to CLAIM_MANAGER_ADMIN;
GRANT select on V_WARRANTY to CLAIM_MANAGER_ADMIN;
GRANT select on V_INSURANCE to CLAIM_MANAGER_ADMIN;
GRANT select on APP_ADMIN.CANCELLED_DELIVERIES to COURIER_MANAGER_ADMIN;
GRANT select on V_PRODUCT to INVENTORY_MANAGER_ADMIN;
GRANT select on V_WARRANTY to INVENTORY_MANAGER_ADMIN;


CREATE INDEX CUST_ID ON ORDERS(CUSTOMER_ID);
CREATE INDEX PRD_ID ON ORDERS(PRODUCT_ID);
CREATE INDEX WAR_ID ON ORDERS(WARRANTY_ID);
